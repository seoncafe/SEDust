...
|---|---|---|
| values | the computed quantities `Q_ext`, `Q_abs`, `Q_sca`, `g`, `albedo`, `C_ext`, `C_abs`, `C_sca`, `K_abs`, and the coordinate axes `lambda`, `a_eff` they are tabulated on | 64-bit float |
...
The six shipped products come to 45.0 MB on disk: astrodust 9.31, dl07 9.77,
...
grain type, except that `GRAIN_J13.DAT` names `CM20` twice with different
radius ranges, so those two are `CM20_plaw-ed` (the large a-C:H/a-C grains) and
