...
**Which branch.** v1.20 is v1.00 **plus** polarization, not a fork. Everything
scalar is identical — the same builders, the same `dust_emission` /
`dust_extinction`, the same `lam_min` extreme-ultraviolet extension, the same
`calc_kext.x` products — so a MoCafe build that only needs total intensity behaves
the same on either branch, and a scalar-only v1.20 run
(`load_polarized_optics=.false.`) returns `Cext`, `Cabs`, `Csca`, `gbar` and the
total SED bit-identical to a polarized build at zero alignment. What v1.20 adds is
the polarized material of this document. The one incidental difference is the
status numbering: what v1.00 calls 3 and 4 (the dielectric function load and the
`lam_min` floor) are 6 and 7 here, because 3, 4 and 5 are taken by the polarized
paths (§3).
...
1. Build the model with the aligned table path:
   ...
   `qpol_*`/`scatmat_path`. The full set is

   | `st` | meaning |
   |---|---|
   | 1 | Q-table load failed |
   | 2 | size-distribution load failed |
   | 3 | aligned scattering table load failed (only when `scatmat_path` was given) |
   | 4 | a polarized Q table requested explicitly through `qpol_path` could not be read |
   | 5 | `load_polarized_optics=.false.` combined with an explicit polarized-optics path |
   | 6 | astrodust dielectric function load failed (EUV band only) |
   | 7 | `lam_min` below the astrodust dielectric function's own shortest wavelength (EUV band only) |
   | 8 | the EUV polarized table could not be read while the grid reaches into that band |
   | 9 | the EUV band of the grid runs outside the EUV polarized table's 0.0124-0.0912 um coverage |

   Codes 6-9 appear only when `lam_min` extends the grid below 0.0912 um. Note
   that an *absent* EUV companion table is not an error: `build_Cpol` reports it
   on stderr and leaves that band's dichroism and birefringence zero (§9), so the
   status seen in practice is 9. Check `st == 0` and `scm_loaded`;
...
## 5. Regenerating the table, and getting other wavelengths
...
### Other scattering wavelengths

Five bands are the intended scope, not an unfinished sweep: polarized transfer is
run at the few wavelengths an observation was made at. The requirement is that a
wavelength MoCafe later needs can be computed, and it can —
`run_scatmat_aligned.x` takes wavelengths on its command line:

```
./run_scatmat_aligned.x 0.44 0.55 0.79
```

Three limitations of that path, stated so they are not met by surprise:

1. **The output stem is fixed.** A custom run writes the same
   `output/scatmat_aligned_astrodust_P0.20_Fe0.00_1.400.dat`, so it **overwrites**
   the shipped five-band table. Move the shipped file aside first.
2. **There is no merge mode.** A band cannot be appended to an existing table;
   every wavelength asked for is recomputed into a new file.
3. **There is no radius-window split and no large-x certification.** The driver
   has no `ja=` window mode, and it applies the plain `X_LARGE = 50` cut without
   the two-setting convergence cross-check that `run_q_jori.x` uses above x = 50.

### Other polarized-extinction wavelengths

The extinction-side optics (`Cpol_ext`, `Cbir_ext`, `Cpol`) are already on the
whole DH21 axis, 0.0912-39810 µm, so nothing is needed there. Wavelengths *below*
0.0912 µm are generated by naming them:
...
- **Path queries are concurrent.** `scatmat_band`, `extinction_matrix_aligned`,
  ...
  wavelength to a band once, so no wavelength search runs in the photon loop.
- **No T-matrix is solved at run time.** The library only reads precomputed
  tables. The reason is that convergence cannot be certified at x >~ 55 from
...
Bands: 0.36, 0.44, 0.55, 0.64, 0.79 µm (§5 covers computing others).

In-core cost after `load_scatmat_aligned`: dominated by `scm_Z`, which is
`nti*nts*nphi*16*nband` doubles — about 81 MB for the production grid, linear in
the number of bands; the F and K arrays add under 0.1 MB.
...
- **Below 0.0912 µm the dichroism and birefringence are zero by omission.** If
  `lam_min` extends the grid past the Lyman limit, `build_Cpol` looks for an
  extreme-ultraviolet companion Q table, and that table **is not shipped**. When
  it is missing the band stays at zero and the reader says so on stderr ("zero by
  omission, not by physics"). Do not read that zero as an asymptote: measured
  first-principles size integrals put the alignment-weighted
  `|C_pol,ext| / C_ext` at 1.26e-3 at the 0.0912 µm seam, *rising* to 3.64e-3 near
  20.6 eV before decaying to 1.6e-4 at 100 eV, with the **sign opposite** to the
  optical band (the reversal is near 0.106 µm); `|C_bir,ext| / C_ext` falls
  monotonically from 7.2e-3 at the seam and changes sign near 51 eV. A sphere has
  exactly zero dichroism and exactly zero birefringence, so scalar EUV optics on
  the volume-equivalent sphere could never have supplied these entries. Generate
  the band with `run_q_jori.x` (§5) if a MoCafe run needs those wavelengths.
