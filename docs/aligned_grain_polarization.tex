...
$Z(\lambda,\theta)$, aligned orientation &
  SEDust & later &
  The fixed-orientation amplitude is now in the build
  (\code{ampl\_oriented.f}), but only the forward value and the
  scattering integral are used; the full aligned matrix at all angles is
  not yet formed. Aligned spheroids scatter anisotropically with respect
  to $\mathbf{B}$, which spherical Mie cannot represent. \\
\Ccirc &
  SEDust & done (optics) &
...
orientation-dependent amplitude matrix is evaluated by \code{AMPL}, now in the
build as \code{ampl\_oriented.f} and already used for the forward amplitude and
the scattering integral; what is missing is retaining it at all scattering angles
and assembling the sixteen Mueller elements. Stated
...
since the release publishes no birefringence. What is still deferred is
returning it through \code{dust\_extinction} and using it in the
$U\leftrightarrow V$ transfer term; the aligned scattering matrix, a
separate and larger item, is where any remaining circular-polarization work
belongs.
