...
\code{data/astrodust/q\_DH21Ad\_P0.20\_Fe0.00\_1.400.dat.gz}
...
\subsection{Wavelength coverage of the polarized optics}
\label{sec:polcoverage}

Two different objects have two different coverages, and confusing them is
easy.

The polarized \emph{extinction} optics --- \Cpol\ (polarized absorption,
which drives the emission), $C_{\rm pol,ext}$ (dichroism) and
$C_{\rm bir,ext}$ (birefringence) --- are tabulated on the full DH21
wavelength axis, all 1129 nodes from $0.0912$ to $39810\um$. Nothing about
them is restricted to a band.

The angle-resolved \emph{scattering} matrices of
\S\ref{sec:alignedscat} are stored at five wavelengths, $0.36$, $0.44$,
$0.55$, $0.64$ and $0.79\um$. That is a deliberate scope, not an
incomplete sweep. Polarized radiative transfer is essentially never run
on a whole wavelength grid: it is run at the few wavelengths an
observation was made at, and five optical bands cover the update this
work exists for. What the design has to guarantee is not that every
wavelength is precomputed, but that a wavelength one later needs
\emph{can} be computed --- and it can, by naming it on the generator's
command line
(\code{sedust\_polarization\_implementation.pdf}, \S7 and \S8).

\subsection{Dichroism below the Lyman limit}
\label{sec:euvdichroism}

The polarized tables stop at $0.0912\um$ because the DH21 wavelength axis
does. A model grid can be extended shortward of it, and when it is, the
dichroic and birefringent entries of the extreme-ultraviolet block are
currently \emph{zero} --- and that zero is a documented deficit, not a
physical statement. The distinction matters because a vanishing
polarization is a plausible-looking asymptote, so it can be mistaken for
a result.

It is not one. First-principles size integrals over the aligned astrodust
population give, for the alignment-weighted ratio,
\begin{equation}
  \frac{|C_{\rm pol,ext}|}{\Cext}
  \;=\;
  \begin{cases}
    1.26\times10^{-3}, & \lambda = 0.0912\um\ \hbox{(the seam)},\\[2pt]
    3.64\times10^{-3}, & 20.6\ \hbox{eV},\\[2pt]
    1.6\times10^{-4},  & 100\ \hbox{eV},
  \end{cases}
  \label{eq:euvpol}
\end{equation}
so the dichroism \emph{rises} by a factor $2.9$ above the seam before it
decays, and it does so with the sign \emph{opposite} to the optical band;
the reversal falls near $0.106\um$. The birefringent ratio
$|C_{\rm bir,ext}|/\Cext$ decreases monotonically from $7.2\times10^{-3}$
at the seam and changes sign near $51$\,eV.

Zero would be the correct limit only where both $x \gg 1$ and
$|m-1| \ll 1$. Neither holds: $|m-1|$ is $0.765$ at the seam, $0.564$ at
$20$\,eV, and drops below $0.1$ only above ${\sim}50$\,eV, while the
grains that carry the signal sit at $x = 5$--$50$.

One structural point follows and is worth stating on its own. A sphere
has \emph{exactly} zero dichroic extinction and \emph{exactly} zero
birefringence, by symmetry. Scalar extreme-ultraviolet optics computed on
the volume-equivalent sphere therefore could not have produced these
entries at any accuracy: the grain shape has to be retained to obtain
them at all, which is why the band needs the orientation-resolved
spheroid calculation rather than an extension of the Mie table.
...
A third file in the same tree, \code{qlib\_gra\_D16MGemt\_1.400}, was for a
time read as though it carried a third convention, with \code{jori = 1}
taken for the \emph{random-orientation average}. It does not: it uses the
same Mishchenko ordering as \code{q\_DH21Ad\_*}, so \code{jori = 1} is
$k \parallel a$ there as well, and \emph{none} of the three indices is the
random-orientation average. That average has to be built,
$\langle Q\rangle_{\rm random} = \tfrac{1}{3}Q_\parallel +
\tfrac{2}{3}Q_\perp = \tfrac{1}{3}\,\code{jori}2 + \tfrac{2}{3}\,\code{jori}3$,
which is what \code{sed/src/q\_graphite\_d16.f90} now does on load; taking
\code{jori = 1} for it instead overstates $Q_{\rm abs}$ by ${\sim}22\%$ in the
Rayleigh limit, because that orientation carries no $Q_\parallel$ at all. A
reader routine still has to dispatch on the file identity rather than on the
index alone, since DF09 and the astrodust tables genuinely differ.
...
against the HD23 release. The \code{tmatrix/} library now provides both the
random-orientation solve (\code{tmatrix\_eval}, and
\code{tmatrix\_eval\_scattering\_matrix} when the angular expansion is wanted)
and Mishchenko's fixed-orientation amplitude routine \code{AMPL},
so SEDust can also regenerate the orientation-resolved cross sections from the
dielectric function itself; the default polarized optics still read the release
table, but they no longer have to. A T-matrix run is therefore useful not only
for the scattering matrix (\S\ref{sec:optical}, now done for five optical bands)
and for exploring other axial ratios or compositions, but for the polarized cross
sections directly. The release table carries only $Q_{\rm ext}$, $Q_{\rm abs}$ and
$Q_{\rm sca}$, so from it \Ccirc\ is zero; the regenerated table adds a birefringence
block (the real part of the forward-amplitude difference) from which \Ccirc\ follows,
certified by Kramers-Kronig against the dichroism.
...
\item \code{../data/astrodust/kext\_astrodust\_MW.dat} carries the polarized
...
of \code{../data/astrodust/kext\_astrodust\_MW.dat}, because that file is already
...
$Z(\lambda,\theta)$, aligned orientation &
  SEDust & \textbf{done} &
  Now built: the full fixed-orientation Mueller matrix
  $Z(\theta_i;\theta_s,\phi)$ is formed at all scattering angles from
  \code{AMPL} and size-integrated over the aligned population,
  in five optical bands (\S\ref{sec:alignedscat}). Aligned spheroids scatter
  anisotropically with respect to $\mathbf{B}$, which spherical Mie cannot
  represent. \\
\Ccirc &
  SEDust & done (optics) &
  Birefringence converts $U$ into $V$. Absent from the release, but now
  computed as the real part of the forward-amplitude difference and stored
  in the regenerated table; Kramers-Kronig certified. Returning it through
  \code{dust\_extinction} and consuming it in transfer is still to do. \\
...
is done. The T-matrix driver in the tree was originally derived from
Mishchenko's \code{tmd.lp.f} by wrapping the implicit main program as a
subroutine, and in the process the final \code{MATR} call was removed and the
angular expansion coefficients were dropped from the returned quantities. The
\code{MATR} routine itself remained in the file, and the call was later
restored. That f77 driver has since been rewritten as the modern-Fortran
library in \code{tmatrix/src/}: the entry point that returns the expansion is
\code{tmatrix\_eval\_scattering\_matrix}, which fills a
\code{tmatrix\_scatmat\_t} (\code{al1}$\ldots$\code{al4}, \code{be1},
\code{be2}, \code{lmax}, \code{nmax\_tm}), and \code{MATR} itself is
\code{SCATMAT\_FROM\_MOMENTS} in module \code{scattering\_matrix\_expansion}.
Mishchenko's unmodified sources are kept under
\code{tmatrix/reference/upstream/} so that every migrated routine stays
checkable line by line against them; nothing there is compiled.
...
\paragraph{Scattering matrix, aligned orientation (built).} The full
orientation-dependent amplitude matrix is evaluated by \code{AMPL}, in
\code{tmatrix/src/fixed\_orientation\_amplitude.f90}. Retaining it at all
scattering angles and assembling
the sixteen Mueller elements --- the step that was missing --- is now done, and
the result is size-integrated over the aligned population into the phase matrix
$Z_{\rm al}(\theta_i;\theta_s,\phi)$ and the extinction matrix $K(\theta_i)$. The
cost turned out to be storage rather than algorithm, as anticipated: the shipped
file spans five optical bands at 138\,MB (\S\ref{sec:alignedscat}). There is no
shortcut through published data --- the HD21 release carries integrated
$Q_{\rm ext}$, $Q_{\rm abs}$ and $Q_{\rm sca}$ only, with no scattering matrix at
any orientation --- so this had to be computed from the T-matrix directly.

\paragraph{Birefringence came free with the aligned extinction, as
predicted.} \Ccirc\ is absent from the release but needs no separate
computation. In the forward-scattering amplitude the difference
$S_{\theta\theta} - S_{\phi\phi}$ carries dichroism in its imaginary part
and birefringence in its real part, so the fixed-orientation amplitude run
already done for \Cpol\ yields \Ccirc\ from the same output. Keeping the
real part is now implemented: the regenerated table carries it as a 4th
block, and it is certified by Kramers-Kronig against the dichroism to a
median $\sim0.1\%$ over the interior of the grid, the only check available
since the release publishes no birefringence. \code{dust\_extinction} now
returns it through an optional \code{Cbir\_ext} argument, and the aligned
scattering table carries the same birefringence in its $K(\theta_i)$
(\S\ref{sec:alignedscat}); what remains is using it in the
$U\leftrightarrow V$ transfer term, which is the host's task.
...
The regenerated orientation-resolved table (the first-principles route added
after v1.20; see \code{sedust\_polarization\_implementation.pdf}) is the concrete
way to unify them. It supplies \Cpol\ from the exact fixed-orientation T-matrix,
$\tfrac12(Q_3-Q_2)$, rather than from the MPFA approximation of
Eq.~(\ref{eq:cpol}), so both channels rest on the same exact optics and the mixed
convention is removed outright, at the price of the FIR emission dichroism
disagreeing with the release by the same few parts in $10^4$. It is opt-in for
exactly this reason, to keep the regression baseline until that step is taken
deliberately.
...
\item Only then bring in the aligned scattering matrix (its supply is now
      built, \S\ref{sec:alignedscat}; what remains is consuming it), \Ccirc,
      the averaging convention, and any extension beyond the optical.
\end{enumerate}
...
%====================================================================
\section{The aligned scattering matrix, as built}
\label{sec:alignedscat}

\emph{Status note (2026 July 20). The item this section describes ---
$Z(\lambda,\theta)$ for aligned orientation, listed as ``later'' in earlier
drafts and now marked \textbf{done} in Table~\ref{tab:optical} --- is built and
verified. Its implementation is documented in
\code{sedust\_polarization\_implementation.pdf} \S8 and its calling interface in
the SEDust user manual; this section records the physics and the place it takes
in the transfer.}

The random-orientation matrix of \S\ref{sec:optical} answers how an
\emph{unaligned} population scatters. An aligned grain scatters differently
depending on how the incident ray is inclined to its symmetry axis, and that
dependence is the scattering-side counterpart of the dichroic extinction the
model already supplies. SEDust now computes it for the astrodust spheroid, from
the same fixed-orientation T-matrix amplitude used for the oriented cross
sections.

\subsection{Why the phase matrix carries three angles}

For a randomly oriented particle with a plane of symmetry the scattering matrix
depends on the single scattering angle $\Theta$ and has six independent elements
(the $F_{11},F_{22},F_{33},F_{44},F_{12},F_{34}$ of \S\ref{sec:optical}). Fix the
grain --- place its symmetry axis $\hat a$ along $z$ --- and that reduction no
longer holds: the matrix depends on the incidence direction relative to the axis
\emph{and} on the full scattering direction. Writing $\theta_i$ for the polar
angle of incidence from the axis and $(\theta_s,\phi)$ for the scattering
direction, axial symmetry leaves only the azimuth \emph{difference} $\phi$, so
\begin{equation}
  Z = Z(\theta_i;\,\theta_s,\phi),\qquad 16\ \hbox{elements},\ \
  \um^2\,\mathrm{sr}^{-1}.
  \label{eq:Zangles}
\end{equation}
Sixteen elements rather than six is the price of dropping the orientation
average, and the extra argument $\theta_i$ is exactly the inclination of the ray
to $\mathbf{B}$ that carries the aligned-scattering information.

\subsection{The meridional basis}

Stokes vectors are referred to Mishchenko's meridional basis of each propagation
direction \emph{in the grain frame}, whose $z$ axis is the alignment axis (the
local $\mathbf{B}$): $\hat e_1 = \hat\theta$ (call it $v$), $\hat e_2 = \hat\phi$
($h$), with $Q = I_v - I_h$. This is the basis into which the fixed-orientation
amplitude matrix $\mathbf{S}$ is naturally rotated, so no adaptor stands between
the analytic dipole limit and the T-matrix regime. At $\theta_i = 90^\circ$ the
$v$ polarization is $\mathbf{E}\parallel\hat a$ (the jori\,$=2$ orientation of
\S\ref{sec:optics}) and $h$ is $\mathbf{E}\perp\hat a$ (jori\,$=3$), which ties
the extinction matrix at edge-on incidence back to the jori cross sections and
fixes the sign of $\Cpol = \tfrac12[C_H(90) - C_E(90)]$ of Eq.~(\ref{eq:cpol}).
The transfer code rotates the photon Stokes vector into this grain frame at each
scattering event and back out afterward.

\subsection{Exact symmetries}

Three symmetries of the oblate spheroid reduce the stored angular ranges and, more
usefully, are exact numerical checks on the matrix construction:
\begin{itemize}
\item \textbf{Azimuth mirror.} The plane through the axis and the incidence
      ...
      $13,14,23,24,31,32,41,42$) change sign. Only $\phi\in[0,180^\circ]$ need be
      stored.
\item \textbf{Equatorial mapping.} Under $z\to-z$, $\theta_i\to180^\circ-\theta_i$
      maps onto the stored range with $\theta_s\to180^\circ-\theta_s$, $\phi$
      unchanged, and the same off-diagonal-block sign flip (the diagonal blocks
      keep their sign). Only $\theta_i\in[0,90^\circ]$ need be stored; physically
      this encodes that the alignment axis is headless.
\item \textbf{Axisymmetry at $\theta_i=0$.} When the ray travels along the axis
      the problem is axisymmetric: $Z$ becomes independent of $\phi$ up to a frame
      rotation and takes the six-element block-diagonal form. This is a check, not
      a saving.
\end{itemize}
All three were verified numerically (to $\sim10^{-15}$ for the equatorial mapping,
a few parts in $10^{7}$ for the azimuth mirror) before being used to reconstruct
the unstored octants.

\subsection{Aligned plus unaligned, exactly}

Partial alignment is handled by the same decomposition used for the extinction: a
perfectly aligned fraction $\falign(a)$ plus a randomly oriented remainder,
\begin{equation}
  Z_{\rm total}(\theta_i;\theta_s,\phi) = \int da\,n(a)\,
    \bigl[\,\falign(a)\,Z_{\rm oriented}(a) + (1-\falign(a))\,F_{\rm random}(a)\,
    \bigr].
  \label{eq:zdecomp}
\end{equation}
SEDust stores the aligned integral $Z_{\rm al} = \int da\,n(a)\,\falign(a)\,
Z_{\rm oriented}(a)$, the random-orientation matrix $F_{\rm tot}$ of every grain,
and its aligned-weighted twin $F_{\rm ref} = \int da\,n(a)\,\falign(a)\,
F_{\rm random}(a)$. Because Eq.~(\ref{eq:zdecomp}) is \emph{linear} in $\falign$,
a cell alignment written as $\falign^{\rm cell} = \eta\,\falign^{\rm ref}$ with one
scalar $\eta$ scales the aligned part exactly, $Z_{\rm al}^{\rm cell} = \eta\,
Z_{\rm al}$, and the unaligned remainder is $F_{\rm unal} = F_{\rm tot} - \eta\,
F_{\rm ref}$ (in absolute units, after restoring each matrix's $C_{\rm sca}$). Three
stored integrals therefore give the scattering optics of any $\eta$ with no
re-integration and no approximation --- the Rayleigh reduction factor $R$ of
\S\ref{sec:alignment} enters as this $\eta$. A genuinely different profile shape
requires regenerating the integrals over size.

\subsection{The extinction matrix from forward amplitudes}

The $4\times4$ extinction matrix of Eq.~(\ref{eq:transfer}) needs $\Cext(\theta_i)$,
the dichroism $\Cpol(\theta_i)$, and the birefringence $\Ccirc(\theta_i)$ at the
ray's inclination to the field. These are read from the \emph{forward} amplitude
at each $\theta_i$ --- the same amplitude call the scattering matrix uses ---
with $\Cext$ from its imaginary part (the optical theorem) and $\Ccirc$ from its
real part, exactly as \S\ref{sec:optics} anticipated for the free birefringence.
Stored on the $\theta_i$ grid, the extinction matrix is thus \emph{exact at every
propagation angle}.

This supersedes the interpolation the three-orientation table forces. The jori
table gives $C_E$ only at $\psi = 0$ and $90^\circ$, so a transfer code has to
interpolate between them with the modified picket fence $\sin^2\psi$ law of
Eq.~(\ref{eq:mpfa}); away from small size parameters that law is only accurate to
about ten percent. The forward-amplitude route removes the interpolation. In the
Rayleigh limit it \emph{reduces} to the $\sin^2$ law exactly: the electric-dipole
cross sections obey
\begin{align}
  C_v(\theta_i) &= \sin^2\theta_i\,C(\mathbf{E}\!\parallel\!\hat a)
                 + \cos^2\theta_i\,C(\mathbf{E}\!\perp\!\hat a), &
  C_h(\theta_i) &= C(\mathbf{E}\!\perp\!\hat a),
  \label{eq:ksin2}
\end{align}
so $\Cpol(\theta_i) = \tfrac12[C_h - C_v] = -\tfrac12\sin^2\theta_i\,
[C(\mathbf{E}\!\parallel\!\hat a) - C(\mathbf{E}\!\perp\!\hat a)]$, which is
Eq.~(\ref{eq:qabspol}) with $R\to\eta$. The dipole $K(\theta_i)$ reproduces
Eq.~(\ref{eq:ksin2}) to machine precision, and the T-matrix $K(\theta_i)$ follows
it to a few parts in $10^4$ at small $x$, which is the anchor that the exact route
and the interpolated one agree where they must.

\subsection{Where each quantity enters the Peest transfer}
\label{sec:alignedscat:map}

The Monte Carlo formalism of Peest et al.\ (2017, \code{2017A\&A...601A..92P}),
extended to aligned spheroids, dichroism and birefringence by Peest et al.\
(2023, \code{2023A\&A...673A.112P}), and the scattering-plus-dichroism transfer of
Seon (2018, \code{2018ApJ...862...87S}) are the destination. Each SEDust quantity
maps onto one step of it (Table~\ref{tab:peestmap}); the division of labor is that
SEDust returns the matrices in the grain frame and MoCafe performs the frame
rotations, the direction sampling, the peel-off, and the $\exp(-K\tau)$ transfer.

\begin{table}[!ht]
\centering
\caption{Which SEDust quantity enters which step of the Peest et al.\ Monte Carlo
transfer. All are read from the aligned scattering table through the library API;
$\theta_i = \arccos(\hat k\!\cdot\!\hat B)$ and the cell alignment scale $\eta$
are the only inputs.}
\label{tab:peestmap}
\begin{tabularx}{\textwidth}{l l X}
\toprule
Transfer step & SEDust quantity & Role \\
\midrule
Optical depth / attenuation &
  $K(\theta_i) = \eta\,K_{\rm al}(\theta_i) + (C_{\rm ext}^{\rm iso} -
   \eta C_{\rm ext,ref})I$ &
  The $4\times4$ extinction operator of Eq.~(\ref{eq:transfer}): $\Cext$ on the
  diagonal, $\Cpol$ in the $IQ$ block (dichroism), $\Ccirc$ antisymmetric in the
  $UV$ block (birefringence). The aligned piece is direction-resolved; the
  isotropic remainder avoids double-counting the aligned population. \\
Albedo, population split &
  \code{scattering\_cross\_sections} $\to$ $C_{\rm sca,al}(\theta_i)$,
  $C_{\rm sca,unal}$ &
  The single-scattering albedo and the probability that an interaction is with the
  aligned versus the unaligned population. \\
Direction sampling &
  $Z_{\rm al,11}(\theta_i;\theta_s,\phi)$ or $F_{\rm unal,11}(\Theta)$ &
  The sky brightness the scattered direction is drawn from --- the aligned phase
  function in the grain frame, or the random-orientation one in the scattering
  plane. \\
Stokes update at a scatter &
  full $Z_{\rm al}$ (aligned) or the six-element $F_{\rm unal}$ (unaligned) &
  Transform the Stokes vector, with meridional rotations into and out of the grain
  frame for the aligned case; the unaligned case works in the scattering plane as
  the existing \code{scatmat} file already does. \\
Peel-off to the observer &
  $Z_{\rm al}$ / $F_{\rm unal}$ toward $\hat n_{\rm obs}$ &
  Evaluate the same phase matrix at the forced scattering geometry and weight by
  the albedo. \\
\bottomrule
\end{tabularx}
\end{table}

The far-infrared and submillimeter milestone needs none of the scattering rows:
there the albedo is essentially zero and only the $K(\theta_i)$ row and the
emission vector are used. The scattering rows are what the optical update of
Seon (2018) additionally requires, and they are now supplied.
