...
as the astrodust model is the target, no T-matrix run is needed at all;
that code becomes relevant only for exploring other axial ratios or
compositions, or for the fixed-orientation amplitude matrix that circular
polarization would require (\S\ref{sec:optics}): the released table
...
Those stages consume optical quantities that SEDust does not yet
produce, and in one case that no released file contains. Stage~1
delivered \Cpol\ and \code{Cpol\_ext}, which are sufficient for
polarized emission and for dichroic extinction, and nothing else.
Table~\ref{tab:optical} lists what is still missing on the supply side,
with the priority each item carries for the single-wavelength update.
...
$Z(\theta)$ at $V$, random orientation &
  SEDust & high &
  \code{MATR} call removed from \code{tmd\_one.f}. Scattering dominates
  the optical opacity, so the phase matrix sets the scattered
  polarization; in the far infrared the albedo is near zero and $Z$
  never enters. \\
...
\paragraph{Scattering matrix, random orientation (needed).} The T-matrix
driver in the tree, \code{tmatrix/src/tmd\_one.f}, was derived from
Mishchenko's \code{tmd.lp.f} by wrapping the implicit main program as a
subroutine. In the process the final \code{MATR} call was removed and
the angular expansion coefficients were dropped from the returned
quantities; the header comment at lines 29--30 records this, and the
argument list ends at \code{QEXT, QSCA, WALB, ASYMM, IERR}. The inner
\code{MATR} routine itself is still present in the same file. Restoring
the call and returning the expansion coefficients recovers
$Z(\theta)$ for randomly oriented spheroids at moderate effort. For the
single-wavelength goal only the $V$ band is required, so what has to be
computed and stored is one $\theta$ grid over the size distribution, not
a wavelength-resolved table. That alone already improves on the 2018
model, whose phase matrix was spherical Mie.
...
\item Test the sensitivity to the \falign\ functional form, step versus
      smooth.
\item Restore the random-orientation scattering matrix in
      \code{tmd\_one.f}, for which the \code{MATR} call is enough. One
      band suffices to begin with, and adding a second or third costs
      another run rather than another method.
