...
$\lambda$ range                            & \SIrange{1e-4}{3.981e4}{\micro\meter} & $Q$-table grid, 1762 points (\S\ref{sec:euv}) \\
...
\texttt{gfortran -I<sed> rt.f90 -L<sed> -lsedust -fopenmp}, and it carries
no T-matrix at all: the astrodust optics, the ionizing band included, are
read off the $Q$ table. \texttt{WITH\_TMATRIX=1 make libsedust.a} adds the
spheroid solver, for the case a band has to be computed after all, and wants
\texttt{cd ../tmatrix \&\& make libtmatrix.a} first. A minimal working
...
The astrodust wavelength grid is the T-matrix $Q$ table's, and the
table reaches \SI{1e-4}{\micro\meter} ---
\SI{12.4}{\kilo\electronvolt}. \textbf{The ionizing band is inside it},
computed the same way as the rest of it, so a host that transports
...
\medskip
\noindent \texttt{build\_astrodust} and \texttt{build\_dl07} still take an
optional \texttt{lam\_min} [\si{\micro\meter}] for a host that needs
something shorter still. Log-spaced points are then prepended from
\texttt{lam\_min} up to the table, at a step no coarser than the
table's own $\Delta\ln\lambda = 0.00794$ at its short-wavelength end,
with the requested floor placed exactly. Omitting \texttt{lam\_min}
prepends nothing, so every earlier result is bit-identical. For
astrodust there is now nothing left to ask for: the table already
reaches the short end of the DH21 dielectric function itself
($1.00003\times10^{-4}$\,\si{\micro\meter}), so every legal
\texttt{lam\_min} either falls inside the table, prepending nothing, or
below the dielectric data, where it is refused. DL07 can still be
extended, to \SI{6.205e-5}{\micro\meter}, because the D03 optical
constants reach further than the table does.
...
\texttt{data/kext\_astrodust\_MW\_euv.dat} held while that route was the
one in use --- the step
...
\texttt{data/kext\_dl07\_MW\_euv.dat} (1823 points, from
...
\midrule
\texttt{kext\_astrodust\_MW.dat} & 1762 & 1e-4--39810 &
the $Q$-table grid, which already covers the ionizing band \\
\texttt{kext\_astrodust\_MW\_euv.dat} & 1762 & 1e-4--39810 &
the same grid: the requested floor is inside it, so nothing is
prepended \\
\texttt{kext\_dl07\_MW.dat} & 1762 & 1e-4--39810 &
the $Q$-table grid, which DL07 shares \\
...
at the table boundary. The two astrodust products now cover the same
grid and read every wavelength of it, the ionizing band included, off
the $Q$ table: \texttt{euv} has nothing left to prepend, and the files
differ only in the field width they are written with and in their
headers. \texttt{kext\_astrodust\_MW.dat} is a tracked regression
reference and is reproduced byte for byte by
\texttt{./calc\_kext.x astrodust}.

\medskip
\noindent The comparison that put the T-matrix in that band, and
eventually in the table, is worth keeping on the record. While the band
was still solved on the fly, the two routes agreed exactly at
...
wavelength grid now reaches into the ionizing band
