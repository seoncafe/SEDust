...
$\lambda$ range, extended                  & \SIrange{1.001e-4}{3.98e4}{\micro\meter} & optional, 1719 points; see \S\ref{sec:euv} \\
\bottomrule
\end{tabularx}
\end{table}
...
Draine's $K_{\rm abs}$. \texttt{data/kext\_astrodust\_MW.dat}, which is
...
nothing, so every earlier result is bit-identical. The same is true of
the other options added at the same time --- the
\texttt{astrodust\_index\_path} of \texttt{sed\_init} /
\texttt{build\_astrodust}, and the \texttt{albedo} output of
\texttt{dust\_extinction}: absent, they change nothing.
...
is reproduced byte for byte by \texttt{./calc\_kext.x astrodust}. The
three extended products are byte-identical to the files of the same
name on the scalar branch (v1.00), which is the check that the
polarized branch did not disturb the scalar optics.

\paragraph{Why the extended products carry no polarized column.}
Shortward of \SI{0.0912}{\micro\meter} the dichroic and birefringent
extinction need the EUV companion of the orientation-resolved table,
covering \SIrange{0.0124}{0.0912}{\micro\meter}. That table is not
distributed --- it is generated on demand --- and without it
\texttt{build\_Cpol} announces on \texttt{stderr} that the band is
``zero by omission, not by physics'' and leaves it at zero. Writing
that zero into a data file would launder a known deficit into a
physical value, so the three extended products stay scalar. When the
...
\texttt{status = 9}, and a table that was requested but cannot be read
with \texttt{status = 8}. Below \SI{0.0124}{\micro\meter}
...
dichroic), and the wavelength grid can be carried into the ionizing
...
the polarized optics stop at the Lyman limit unless the EUV companion
table is generated.
