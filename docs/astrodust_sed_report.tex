...
$\lambda$ range, extended                  & \SIrange{1.001e-4}{3.98e4}{\micro\meter} & optional, 1719 points (\S\ref{sec:euv}) \\
\bottomrule
\end{tabularx}
\end{table}
...
only on $\lambda$ (not on $T$ or grain size); the measured ratios of
all three astrodust stages (S1\_C1, S1\_C2, S2) and PAH agree with one
another to better than $10^{-7}$, i.e.\ to the precision the
nine-significant-digit output files carry, even though the S1\_C2 SED
itself differs from S1\_C1 by orders of magnitude. The growth at
$\lambda \gtrsim$~\SI{1}{\milli\meter} is driven
by the CMB component of $J_\lambda$ via
$J_{\rm CMB}/B_{\rm env}(\lambda) =
1/(\mathrm{e}^{hc/\lambda k T_{\rm CMB}} - 1)$ with
$T_{\rm CMB} = \SI{2.725}{K}$; the diluted-blackbody part of the
Mathis field supplies the whole (utterly negligible) factor at
\SI{100}{\micro\meter} and nothing measurable at longer wavelengths.
...
Numerically the measured boost reproduces the analytic prediction to
$2\times10^{-9}$ in relative terms at 3 and \SI{5}{\milli\meter} and
$2\times10^{-7}$ at \SI{1}{\milli\meter}; across the 404 wavelengths
where the factor exceeds $10^{-6}$ the median relative deviation is
$4\times10^{-9}$, which is the round-off of the nine-significant-digit
output files rather than anything in the calculation. The
...
two stages are essentially identical). That comparison was made in the
literal-Mathis configuration of the next subsection (\SI{2.9}{K} CMB,
$10^{-13}$ dilution at \SI{4000}{K}), which is where its $-15.83\%$
baseline FIR-peak deficit comes from; it has not been re-measured with
the production field, so read its columns against each other rather
...
This section is a \emph{verification of the T-matrix optics} against
the HD23 release, carried out by a standalone utility. It is not a
library capability: this tree is the scalar branch (v1.00), whose API
(\texttt{dust\_emission}, \texttt{dust\_extinction}) is unpolarized
throughout. The dichroic extinction matrix and the aligned-grain
scattering matrix live in the separate polarized branch (v1.20).
...
[, albedo] [, status])} is therefore the extinction twin of
\texttt{dust\_emission}: it returns the size-distribution-integrated
cross sections per H on the model's own grid \texttt{m\%lam}, as the
plain binned sums
...
\paragraph{Linking and validation.} \texttt{make libtmatrix.a} then
\texttt{make libsedust.a} produce two static archives that an external Fortran
RT links with \texttt{gfortran -I<sed> -I<tmatrix> rt.f90 -L<sed> -lsedust
-L<tmatrix> -ltmatrix -fopenmp}; the T-matrix archive is required because the
astrodust ionizing band computes its optics with it. A minimal working
...
\subsection{One driver, four models}
...
\end{verbatim}
The columns are $\lambda$\,[\si{\micro\meter}], albedo,
$\langle\cos\theta\rangle$, $C_{\rm ext}$/H, $C_{\rm abs}$/H,
$C_{\rm sca}$/H, with a trailing $K_{\rm abs}$\,[\si{cm^2/g}] where the
model supplies its dust mass per H. The first four are in the order of
Draine's \texttt{kext\_albedo} tables, so a reader written for those
files parses these unchanged; the fifth is not, being $C_{\rm abs}$/H
rather than Draine's $K_{\rm abs}$.

\subsection{Extending the grid below the Lyman limit}

The astrodust wavelength grid is the T-matrix $Q$ table's, and that
table stops at \SI{0.0912}{\micro\meter} --- \SI{13.6}{\electronvolt},
the Lyman limit. A host that transports ionizing radiation needs
shorter wavelengths, so \texttt{build\_astrodust} and
\texttt{build\_dl07} take an optional \texttt{lam\_min}
[\si{\micro\meter}] and prepend log-spaced points from it up to the
table, at a step no coarser than the table's own
$\Delta\ln\lambda = 0.01156$. Omitting \texttt{lam\_min} prepends
nothing, so every earlier result is bit-identical.

In the extended band the astrodust optics come from the DH21 dielectric
function, solved by the T-matrix for the same $b/a = 1.4$ oblate
spheroid and the same random-orientation average as the table itself,
so the model changes neither composition nor shape at the seam.
Bohren--Huffman Mie for the volume-equivalent \emph{sphere} remains
available as a far cheaper approximation, selected with
\texttt{euv\_tmatrix=.false.}; the next subsection is the measurement
of what that substitution costs. The carbonaceous
...
The $Q$ table is a random-orientation T-matrix solution for a
...
\texttt{data/kext\_astrodust\_MW\_euv.dat} held before it was
regenerated for the default route --- the step
across \SI{0.0912}{\micro\meter} is $-1.54\%$ in $C_{\rm ext}$ and
$-1.90\%$ in $C_{\rm abs}$, \emph{smaller} than the neighboring steps
of the table's own grid ($-1.95\%$ / $-1.72\%$ and $-2.20\%$ /
$-1.94\%$), so absorption and extinction join without a visible
discontinuity. Scattering does show one: $C_{\rm sca}$ steps $+0.74\%$
where its neighbors step $-0.39\%$ and $-0.36\%$, and the albedo
$+2.31\%$ against $+1.59\%$ and $+1.39\%$ --- a seam of roughly one
percentage point, consistent with the $2\%$ shape error being carried
mostly by $Q_{\rm sca}$.

Grain by grain, the default T-matrix route removes that seam almost
...
$C_{\rm abs}$ has median $0.012\%$ and maximum $0.068\%$ for the
T-matrix, against median $1.17\%$ and maximum $15.7\%$ for the sphere.
The asymmetry parameter behaves the same way: the table returns
$\langle\cos\theta\rangle = 0$ wherever its own regime bounds place a
radius in the Rayleigh or geometric-optics limit, which the default
reproduces exactly, while the sphere route steps from $0.90$ to $0$
across the seam for $a\gtrsim\SI{1}{\micro\meter}$.
...
wavelength grid.} That distinction is invisible for astrodust and DL07,
whose $Q$ table stops at the Lyman limit itself:
$hc/\SI{0.0912}{\micro\meter} = \SI{13.595}{\electronvolt} <
\SI{13.6}{\electronvolt}$, so the maximum returns the old constant and
nothing in this report moves. It is not invisible for Zubko, whose
DustEM tables start at \SI{1e-3}{\micro\meter}: measuring the grid
there gives \SI{1239.8}{\electronvolt}, ninety-one times the Lyman
limit, for a Mathis field that carries no photon below
\SI{0.0912}{\micro\meter}.
...
form survived review. \texttt{sed/src/main\_zubko.f90} closes that gap
and is built by the default \texttt{make}: \texttt{./main\_zubko.x}
heats the ZDA model with a Mathis field that stops at the Lyman limit,
\texttt{./main\_zubko.x euv} adds a hard component below it, and each
prints the grid candidate against the field candidate ---
...
\texttt{data/kext\_dl07\_MW\_euv.dat} at the reference's wavelengths
...
$0.056$--$0.86\%$, mean $|\Delta\,\text{albedo}| \le 0.0019$ and mean
$|\Delta\langle\cos\theta\rangle| \le 0.00054$. The excluded points
cluster at the X-ray absorption edges (Fe\,K, Si\,K, Mg\,K, Fe\,L,
O\,K, C\,K), which the reference brackets far more finely than our
$\Delta\ln\lambda = 0.0116$ grid resolves. The largest single
...
the $Q$-table grid; no extension requested \\
\texttt{kext\_astrodust\_MW\_euv.dat} & 1719 & 1.001e-4--39810 &
the DH21 astrodust dielectric function, which stops at
$1.00003\times10^{-4}$ \\
\texttt{kext\_dl07\_MW\_euv.dat} & 1761 & 6.205e-5--39810 &
the D03 dielectric functions; the shorter-reaching of astrosilicate and
graphite stops at $6.1992\times10^{-5}$ \\
\texttt{kext\_zubko\_BARE\_GR\_S.dat} & 1201 & 1e-3--1e4 &
...
below it is refused rather than served with a refractive index frozen
at the table boundary. \texttt{kext\_astrodust\_MW.dat} is a tracked
regression reference and is reproduced byte for byte by
\texttt{./calc\_kext.x astrodust}, which builds no extended band and is
therefore untouched by the T-matrix route.
\texttt{kext\_astrodust\_MW\_euv.dat} is integrated with that route: its
$1129$ rows at $\lambda\ge\SI{0.0912}{\micro\meter}$ are byte-identical
to the sphere-route version, since both read them off the $Q$ table,
and below the seam the sphere route was low in $C_{\rm abs}$ by up to
$3.6\%$, high in $C_{\rm sca}$ by up to $4.6\%$ and in albedo by up to
$5.1\%$, and wrong in $\langle\cos\theta\rangle$ by up to a factor of
four at the shortest wavelengths. $C_{\rm ext}$ moved by $0.72\%$.
...
\begin{itemize}
  ...
  \item \textbf{Induced-emission factor $(1+J_\lambda/B_{\rm env})$:}
        ...
        below $10^{-7}$ at $\lambda \le$~\SI{300}{\micro\meter},
...
supplies the transport optics (\texttt{dust\_extinction}), and the
wavelength grid can be carried into the ionizing band
(\S\ref{sec:euv}) --- with the two caveats recorded there: the
astrodust extreme-ultraviolet band has no external reference, and
hard-photon stochastic \emph{emission} is not yet a solved problem.
