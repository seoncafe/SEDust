...
in $C_{\rm abs}$, \emph{smaller} than the neighbouring steps of the
...
neighbours step $-0.39\%$ and $-0.36\%$, and the albedo $+2.31\%$
...
its enthalpy window from the most energetic photon the grid carries.
That was a hard-coded \SI{13.6}{\electronvolt}; it is now
$\max(13.6\,\mathrm{eV},\ hc/\lambda_1)$ taken from the grid itself. On
the unextended grid $hc/\SI{0.0912}{\micro\meter} =
\SI{13.595}{\electronvolt} < \SI{13.6}{\electronvolt}$, so the maximum
returns the old constant and nothing changes there.
