...
\begin{verbatim}
   call sed_init(qtable_path, NT, T_lo, T_hi)
   ...
   do icell = 1, ncells
...
$10^{-13}$ dilution at \SI{4000}{K}), and predates the enthalpy
ghost-bin correction, which together is where its $-15.83\%$
...
than against the table above or against Table~\ref{tab:mathis_c}.
...
a physical quantity for every model, not just astrodust. No population of a shipped model leaves those
arrays unallocated: the PAHs scatter through their graphite fraction
$\xi_{\rm gra}(a)$ of HD23 equation~(15), which is the only part of a
PAH that has a scattering cross section at all.
...
at the table boundary. The \texttt{euv} astrodust product reads every
wavelength of the \texttt{\_euv} $Q$ table, the ionizing band included,
so \texttt{euv} has nothing left to prepend; each non-EUV product is
its EUV counterpart's row subset from the Lyman limit on --- same grid,
same numbers, over the overlap. \texttt{kext\_astrodust\_MW.dat} is a
tracked regression reference, written through its own narrower field
widths, and is reproduced byte for byte by
