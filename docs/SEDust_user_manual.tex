...
\code{SEDust/sed/}. Because those \code{.mod} files share one directory, the
\code{Makefile} carries \code{.NOTPARALLEL} and builds serially; \code{make -j}
is not supported until the object and module graph lives in a separate directory.
