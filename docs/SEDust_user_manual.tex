...
Two archives, built in this order:
\begin{lstlisting}[style=sh]
cd SEDust/tmatrix && make libtmatrix.a   # T-matrix, needed by the EUV band
cd SEDust/sed     && make libsedust.a    # the library archive
\end{lstlisting}
\code{libtmatrix.a} must exist first: the astrodust extreme-ultraviolet band
(\S\ref{sec:euv}) computes its optics with the T-matrix, so every SEDust target
that links \code{sed\_astrodust.f90} links it too. The \code{sed} \code{Makefile}
stops with an instruction rather than a link error when it is missing.
...
\item \code{euv\_tmatrix} (optional, astrodust only, default \code{.true.}):
      ...
      T-matrix on the same $b/a = 1.400$ oblate spheroid as the $Q$ table;
      \code{.false.} substitutes the volume-equivalent-sphere Mie
      approximation, which is far cheaper and about 2\% low in the
      geometric-optics limit. See \S\ref{sec:euv-shape}. Ignored when
...
extended one merely carried below $0.0912$\um, and their nodes coincide over the
overlap. One default therefore serves a host that transports ionizing radiation
and one that does not; it is the \emph{narrower} non-EUV table that has to be
named explicitly. The Zubko model has no EUV extension and needs none --- its
own optics table already reaches \SI{1.24}{\kilo\electronvolt}.}
...
is integrated with the default T-matrix route, so the extinction it serves and
the emission the model computes are one calculation of one particle at every
wavelength. It was regenerated for that change; the $1129$ rows at
$\lambda\ge0.0912$\um\ are byte-identical to the sphere-route version, because
those come from the $Q$ table either way. Below the seam the sphere route was
...
Compile your RT source against \emph{both} archives, with \code{.mod} search
paths pointing at \code{SEDust/sed/} and \code{SEDust/tmatrix/}:
\begin{lstlisting}[style=sh]
gfortran -I<sed> -I<tmatrix> my_rt.f90 \
         -L<sed> -lsedust -L<tmatrix> -ltmatrix -fopenmp -o my_rt.x
\end{lstlisting}
\code{libtmatrix.a} entered the contract when the astrodust EUV band moved to
the T-matrix (\S\ref{sec:euv-shape}); a host that links \code{-lsedust} alone
now fails at link time. The archive itself needs no OpenMP runtime --- the
threading is on the SEDust side --- so \code{-fopenmp} is required only because
\code{libsedust.a} uses it.
...
\code{kext\_astrodust\_MW\_euv.dat}$^\dagger$ & 1719 & $1.001\times10^{-4}$--$39810$ &
the DH21 astrodust dielectric function, whose own table stops at
$1.00003\times10^{-4}$ \\
\code{kext\_dl07\_MW.dat}          & 1129 & $0.0912$--$39810$ &
the T-matrix $Q$-table grid (no extension requested) \\
\code{kext\_dl07\_MW\_euv.dat}$^\dagger$ & 1761 & $6.205\times10^{-5}$--$39810$ &
