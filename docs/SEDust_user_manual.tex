...
\item \textbf{Circular polarization: available only from the regenerated table.}
      ...
      birefringence block, and \code{q\_table\_jori} exposes it as
      \code{qbir\_ext} (with a \code{has\_bir} flag); \code{dust\_extinction} does
      not yet return it, so consuming it in the $U\leftrightarrow V$ transfer term
      is currently the host's responsibility.
\item \textbf{No scattering by aligned grains.} The release gives the
      integrated $Q_{\rm sca}$ with no angular information, so a Mueller matrix
      for an aligned spheroid cannot be built from it.
\item \textbf{Far-infrared and submillimeter polarized emission is complete
      without either.} The albedo there is essentially zero, so the transfer
      needs only the extinction matrix and the emission vector, both fully
      determined by what is exported here.
\end{itemize}
