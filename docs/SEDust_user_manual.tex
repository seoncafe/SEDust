...
\item \code{euv\_tmatrix} (optional, astrodust only, default \code{.true.}):
      ...
      Ignored when \code{lam\_min} asks for no extension, which with the shipped
      $Q$ table is every legal request, so \textbf{this argument currently
      selects nothing}. It is kept for a model whose optics table stops short of
      its own dielectric function.
\end{itemize}
...
since the latter's grid is a subset of the former's, on the same nodes. Now that
the $Q$ table itself reaches \SI{12.4}{\kilo\electronvolt}, the astrodust pair
is the same curve on the same $1762$ wavelengths and the choice no longer
matters there --- \code{kext\_astrodust\_MW.dat} differs only in its narrower
field widths and its extra dichroic column (\S\ref{sec:standalone}). The DL07
pair still differs, by the $61$ points its D03 optical constants reach past the
table. Naming a \code{kext\_path} is otherwise how you point at a
...
$1762$ wavelengths are the T-matrix $Q$-table grid, which is exactly what the
default table was written on, so every model wavelength coincides with a table
node and nothing is interpolated. The one case that still interpolates is a DL07
model built with a \code{lam\_min} below the table, whose prepended points are
log-spaced from that floor and so do \emph{not} share the nodes of
\code{kext\_dl07\_MW\_euv.dat}; a host that needs that band at full fidelity
should generate a table at its own \code{lam\_min}
(\code{./calc\_kext.x dl07 <lam\_min>}) and pass it as \code{kext\_path}.
Astrodust cannot reach that case at all: no legal \code{lam\_min} prepends
anything (\S\ref{sec:euv}).
...
that range. That is the \emph{long-wavelength} part of the model grid, not all
of it: the scalar $Q$ table was carried into the ionizing band and the
orientation-resolved one was not, which leaves the 633 shortest wavelengths of
the model without polarized entries (\S\ref{sec:euv-pol}). The
...
file is the only product that carries it. It is zero over the file's $633$ rows
below $0.0912$\um, where the orientation-resolved table does not reach; that
zero is the announced deficit of \S\ref{sec:euv-pol}, not a value.}
\label{tab:kextcols}
\end{table}
...
returns to what that leaves the injected route for. The
...
On the shipped $1762$-point grid the polarized block therefore covers the last
...
It is no longer the \emph{narrower} product: since the $Q$ table was carried
into the ionizing band it sits on the same $1762$ wavelengths as
\code{kext\_astrodust\_MW\_euv.dat} and agrees with it over the transport
columns to the $5\times10^{-8}$ its narrower fields can express, so what
separates the two files is the eighth column and the field widths this one
keeps as a regression reference.

\noindent \textbf{Its $633$ rows below $0.0912$\um\ carry the announced zero of
\S\ref{sec:euv-pol} in that column, not a measurement.} A host that reads
column 8 must treat that block as absent rather than as small; the polarized
extinction there is of order $10^{-3}$ of $C_{\rm ext}$ and reverses sign, so
zero is not even the right order. The \code{\_euv} products are written without
the column for the same reason --- a deficit written into a data file is one a
reader cannot tell from a result --- and are byte-for-byte the same files the
scalar branch (v1.00) writes, which is also the cleanest demonstration that the
scalar path is identical in the two branches.
...
\item \code{use\_dustlib.f90} --- the scalar host flow, and the one to read
      ...
      radiation asks for a wider grid. Its commentary predates the $Q$-table
      extension: that second build now returns the \emph{same} $1762$-point
      grid as the first, because the table already covers the band it asks for
      (\S\ref{sec:euv}). It is byte-for-byte the same file in the scalar branch
      (v1.00).
...
\item astrodust / DL07: the T-matrix $Q$-table
      (\code{../tmatrix/output/q\_astrodust\_*.dat}), on the axis
      \code{../data/dielectric/DH21\_wave\_to\_12keV}, and the HD23 release size
...
\item dielectric functions: \code{../data/dielectric/} --- the DH21 astrodust
      ...
      computed from, and which fixes how far \code{lam\_min} may reach), the D03
      astrosilicate and graphite indices, and the D16 turbostratic-graphite
      sphere table.
...
\item Polarized optics: \code{../data/dielectric/q\_DH21Ad\_*.dat.gz} (the HD23
      ...
      matrix), with their grid axes \code{../data/dielectric/DH21\_wave} (1129
      wavelengths --- the polarized products stayed on it when the scalar table
      moved to the longer axis) and \code{DH21\_aeff}. The EUV companion table
...
\item Size-integrated transport optics, written by \code{calc\_kext.x}, see
      \S\ref{sec:standalone} --- \code{../data/kext\_astrodust\_MW.dat} (1762
      rows, $10^{-4}$--$39810$\um, with the dichroic 8th column),
      \code{kext\_astrodust\_MW\_euv.dat} (1762 rows, the same range),
      \code{kext\_dl07\_MW.dat} (1762 rows, the same range),
      ...
      The two astrodust products now cover the same grid and the same curve;
      \code{kext\_dl07\_MW.dat} is the row subset of its EUV counterpart that
      starts at the table.
\end{itemize}
