...
The values reproduce \code{data/kext\_astrodust\_MW.dat}
(Table~\ref{tab:kextcols}) to the precision with which that file is written,
because the file is written by this very call, see \S\ref{sec:standalone}
--- \code{calc\_kext.x} builds the model and calls \code{dust\_extinction}, so the
tables under \code{data/} \emph{are} the optics an RT host gets in memory. The
difference is only that the library returns the result on \code{m\%lam}
directly, so a host using its own wavelength grid need not interpolate off the
grid the driver happened to write.
...
dichroic extinction as an eighth column (Table~\ref{tab:kextcols}). It remains the right choice for a
...
column when the model supplies its dust mass per H, and --- for
\code{kext\_astrodust\_MW.dat} only --- an eighth column $C_{\rm polext}$/H
...
this as the current policy rather than a permanent one.
...
\code{ja=JA1:JA2} splits the 169 radii across \emph{processes}, not threads:
Mishchenko's solver hands the converged T-matrix to \code{AMPL} through
\code{COMMON /TMAT/} and keeps further working storage in COMMON blocks, two of
them blank, so the core is not re-entrant. Measured on this machine: 9m22s of
...
\item \code{use\_dustlib.f90} --- the scalar host flow, and the one to read
      first: build a model once, take the transport optics from
...
\item Size-integrated transport optics, written by \code{calc\_kext.x}, see
      ...
      $1.001\times10^{-4}$\um), \code{kext\_dl07\_MW\_euv.dat} (1761 rows, from
      $6.205\times10^{-5}$\um) and \code{kext\_zubko\_BARE\_GR\_S.dat} (1201
      rows, $10^{-3}$--$10^{4}$\um). These are for hosts that read a file; a
      host that links the library should take the same curves from
      \code{dust\_extinction} instead, see \S\ref{sec:ext}
\end{itemize}
