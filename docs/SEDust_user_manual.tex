...
\end{align*}
A population that carries no scattering optics contributes zero to those terms.
In the astrodust model the PAHs are exactly that case: they enter through
absorption only, and the astrodust grains carry all the scattering and the
asymmetry.

The values agree with the corresponding columns of
\code{data/kext\_astrodust\_MW.dat} to the precision with which that file is
written (the cross sections to a few parts in $10^{8}$; the albedo and
$\langle\cos\theta\rangle$ to the file's six written decimals), because both
evaluate the same size integral over the same optics. The difference is that
the accessor returns the result on \code{m\%lam} directly, so a host using its
own wavelength grid need not interpolate off the grid the driver happened to
write.
...
extinction-side scattering optics, so for those models \code{Csca} and
\code{gbar} come back as zeros while \code{Cabs} (and hence \code{Cext}) is
still the correct absorption sum.
