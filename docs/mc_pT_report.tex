...
\item \verb|main_mc_sed.f90|: size-distribution SED driver
  ...
  It calls \verb|sed_init| without the optional \verb|lam_min|, so the
  wavelength grid is the plain T-matrix $Q$-table grid
  (\SIrange{0.0912}{3.98e4}{\micro\meter}) and the extreme-ultraviolet
  extension available to the library does not enter here.
\end{itemize}
...
key & enthalpy model \\
\midrule
\texttt{gra\_dl01} & DL01 \texttt{Car0} graphite (Debye-2 with $\Theta_1=863$ K, $\Theta_2=2504$ K, plus C--H modes for $n_{\rm atom}\le5.75\times10^4$, i.e.\ $a\lesssim\SI{50}{\angstrom}$ --- the Draine \& Li 2001 threshold).  \textbf{Default for graphite and PAH.} \\
...
size distribution (evaluated by \verb|sed_astrodust_mod::sed_init| from
the HD23 analytic forms in \verb|size_dist.f90|) and produces the emission
...
pipeline \verb|calc_sed.x astrodust|.  Both implementations read the
same Q table (\verb|q_astrodust_P0.20_Fe0.00_1.400.dat|), the same
HD23 size distribution (\verb|size_dist.f90|), the same
...
\item[Default (\texttt{F}).]  Use for production runs.  This is the
  ...
  Total wall time on 12 cores is comparable to \texttt{calc\_sed.x astrodust}
...
comparison, \verb|calc_sed.x astrodust| (single-threaded) takes $\sim 10$~s.
...
(no change from the legacy behavior) and $\lambda_c \approx 0.09\,\mu$m
