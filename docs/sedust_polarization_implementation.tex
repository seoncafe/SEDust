...
Eq.~(\ref{eq:falign}). Publishes \code{qpol\_ext}, \code{qpol\_abs},
\code{qran\_*} and the raw \code{qext\_j}/\code{qabs\_j}/\code{qsca\_j}. \\[2pt]
...
\paragraph{Verdict.} The mixture is harmless for polarized \emph{emission},
...
directory. The failure path now calls \code{cleanup\_scratch} before bailing
out, like every other error exit. The general shape of the bug, an error path
that skips the cleanup the success path performs, is worth watching for in any
...
raw arrays \code{qext\_j}, \code{qabs\_j}, \code{qsca\_j} & 13.7\,MB resident \\
derived \code{qpol\_*}, \code{qran\_*} & 7.6\,MB resident \\
time                     & about 1\,s of the 3.1\,s model build \\
\bottomrule
\end{tabular}
\caption{Cost of loading the orientation-resolved table.}
\label{tab:cost}
\end{table}

The cheapest improvement, if this ever matters, is to release
\code{qext\_j}, \code{qabs\_j} and \code{qsca\_j} once the combinations of
Eqs.~(\ref{eq:qpol})--(\ref{eq:qran}) have been formed. Nothing downstream of
\code{build\_Cpol} reads them, and dropping them removes 13.7\,MB, about
two-thirds of the resident total. They are currently kept public because
\code{calc\_polext.f90} and ad-hoc checking programs read the orientation
blocks directly; a release step would have to be opt-in or come after those
