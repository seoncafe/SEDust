...
wavelength axis       & \code{data/astrodust/DH21\_wave}, 1129 nodes \\
size axis             & \code{data/astrodust/DH21\_aeff}, 169 nodes \\
...
\code{data/astrodust/kext\_astrodust\_MW.dat} as
...
\code{data/<model>/kext\_*.dat} table, attached to the model by the builder (optional
...
(\code{data/astrodust/DH21\_wave\_euv}), so it cannot be combined with
...
\code{data/astrodust/q\_DH21Ad\_P0.20\_Fe0.00\_1.400.dat.gz}, or, through
...
8 & the polarized $Q$ table's wavelength grid does not sit node for node on
    the long-wavelength end of the model's grid (wrong count, or a node that
    differs) \\
...
10 & an explicitly named extinction table (\code{kext\_path}) could not be
    read. It is $10$ here and $5$ elsewhere because this builder's polarized
    codes already occupy $3$, $4$, $5$, $8$ and $9$ \\
11 & \code{euv\_tmatrix = .true.} but the spheroid optics of a band below the
    $Q$ table are not available --- unreachable with the shipped table, which
    leaves no such band \\
\bottomrule
\end{tabular}
...
An \emph{absent} extreme-ultraviolet companion table is not an error:
\code{build\_Cpol} reports it on stderr and leaves that band zero
(\S\ref{sec:euvpol}), and the build succeeds. The two codes that band can raise
are $9$, when a companion table that \emph{was} read does not cover the
requested grid, and $8$, when the main polarized table and the model grid
disagree node for node. An implicitly defaulted main polarized table that
cannot be read degrades to zero polarized optics; only one named through
\code{qpol\_path} raises $4$.
...
Measured on \code{calc\_sed.x astrodust}, the maximum resident set size falls from
...
./calc_kext.x astrodust      # -> ../data/astrodust/kext_astrodust_MW.dat, 8 columns
