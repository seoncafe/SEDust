...
\paragraph{No circular polarization or birefringence.} The table stores
$Q_{\rm ext}$, $Q_{\rm abs}$ and $Q_{\rm sca}$ only, none of which is a phase
retardation, so $C_{\rm circ}$ cannot be formed from it and is set to zero.
Since \S\ref{sec:firstprinciples} the fixed-orientation amplitude matrix
\emph{is} computed here (Mishchenko's \code{AMPL}, extracted into
\code{tmatrix/src/ampl\_oriented.f}), but only its imaginary part in the forward
direction is used, for the extinction cross section. The birefringence is the
difference of the complex forward amplitudes for the two linear polarizations;
forming it would extend the oriented pipeline to output that complex amplitude
rather than the real $Q$ it now reduces to. It is no longer a matter of bringing
