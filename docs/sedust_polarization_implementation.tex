...
side. Threads cannot be used: Mishchenko's solver hands the converged T-matrix to
\code{AMPL} through \code{COMMON /TMAT/} and keeps its working storage in further
\code{COMMON} blocks (two of them blank), so the core is not re-entrant. Separate
processes each get their own copy, and a merged file has been checked byte for
