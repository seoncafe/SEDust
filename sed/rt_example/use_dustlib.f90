program use_dustlib
   ...
   !
   ! POLARIZATION IS NOT TABULATED.  This build of the library also serves the
   ! dichroic Cpol_ext and the birefringent Cbir_ext through the same
   ! dust_extinction call, and those two are NOT read from the table: they are
   ! computed on every call, because their alignment weight f_align(a) is
   ! runtime state a host resets cell by cell through dust_set_alignment, which
   ! a table fixed at build time could not follow.  A host that resets alignment
   ! and calls again therefore gets CHANGED polarized outputs and UNCHANGED
   ! scalar ones.  That asymmetry is also why size_integrated_extinction is no
   ! more attractive to a polarized host than to this one: dust_extinction
   ! already fills each half from the right place.  This example asks for
   ! neither polarized output; see rt_example/use_dustlib_pol.f90.
   !
   ...
   ! is a contract: a file that cannot be read fails the build (status 10 for
   ! this builder, whose codes 3, 4, 5, 8 and 9 are already taken by the
   ! polarized optics; the builders without them report 5), which is what a host
   ! wants when the file it was configured with is missing.  Omitting the
   ! argument falls to the model's default, ../data/astrodust/sedust_astrodust.h5.
   ! A default that cannot be read is only an offer -- it leaves the model with
   ! no extinction to serve, and dust_extinction then reports status 2 -- but
   ...
   ! wider ionizing grid.  Polarized optics are recomputed on every
   ! dust_extinction call, rather than read from this scalar table.
   ...
   !
   ! The polarized half is the exception, and the build says so on stderr: the
   ! orientation-resolved table stops at 0.0912 um, so Cpol_ext and Cbir_ext
   ! are zero shortward of it BY OMISSION, not by physics.  A host that needs
   ! them there generates the EUV companion table (tmatrix/driver/run_q_jori.f90,
   ! `euv` mode) and passes qpol_euv_path / qpol_euv_wave_path.
   !
   ! lam_min, the argument that carries a model's grid below its Q table, can
   ! do nothing here.  There is no value that would: at or above 1.0e-4 um the
   ! request already lies inside the table and no wavelength is prepended, and
   ! below it the build is refused, because the DH21 astrodust dielectric
   ! function the prepended band would have to be computed from itself stops at
   ! 1.000032e-4 um.  (Which refusal depends on the build: with the plain
   ! archive this program links it is status 11, no spheroid optics registered
   ! for the EUV band; with euv_tmatrix = .false. it is status 7, lam_min below
   ! the dielectric function.)  Step (5) is where lam_min still does the work
   ! it was written for.
   ...
   ! refused through status 7 rather than served with a frozen refractive
   ! index.  Everything else about the API is unchanged, and omitting lam_min
   ! leaves the grid on the table's own 1762 wavelengths.  This model has no
   ! polarized optics at any wavelength, so the paragraph above does not apply
   ! to it: dust_extinction returns its scalars and nothing else.
   !
