program use_dustlib
   ...
   ! wants when the file it was configured with is missing.  The file named here
   ! is the size integral computed on the T-matrix Q table's own grid, which is
   ! exactly the range this model covers -- the choice for a host that stops at
   ! the Lyman limit.  Omitting the argument falls to the model's default,
   ! ../data/kext_astrodust_MW_euv.dat: the same size integral carried further
   ! into the ultraviolet, so it CONTAINS this range and serves the plain grid
   ! just as well, which is why step (4) leaves it to the default.  A default
   ! that cannot be read is only an offer -- it leaves the model with no
   ! extinction to serve, and dust_extinction then reports status 2 -- but it
