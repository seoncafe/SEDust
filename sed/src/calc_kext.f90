program calc_kext
   ...
   ! wavelength-grid axis (and, for zubko, that model's size-distribution and
   ! optics-set axes), and nothing else.  A word of any other axis -- a solver,
   ! a radiation field, an emission term -- is refused by name, because a
   ! size-integrated transport curve has no referent for one.  Nor is the
   ! graphite of the PAH xi blend offered: these products ARE the model
   ! definition, and a variant of it is a different model, which is what the
   ! `dl07` / `ld01` subject words are for.
   ...
   ! read; zubko adds its own two once the subject is known.
   ...
   case ('themis', 'g18d')
      ...
      ! agree bit for bit, check_build_dust.x measuring that.
...
contains
   ...
   subroutine write_kext_h5(model)
      ...
      ! The MODEL whose product this is -- 'dl07' for both vintages of the
      ! DL07 model -- not the word the command line used.  Which curve inside
      ! that product is kext_tag()'s business.
   ...
   subroutine header_astrodust_qtable_grid()
      ! FROZEN.  These lines reproduce ../data/astrodust/kext_astrodust_MW.dat exactly as
