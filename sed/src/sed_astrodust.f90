...
contains
   ...
   ! =====================================================================
   subroutine sed_init(qtable_path, NT_in, T_lo, T_hi, status, lam_min, &
   ...
   real(wp) function d03_euv_lambda_floor() result(lam_min)
      ...
      ! DL07 and MRN both.  Both materials
   ...
   subroutine build_astrodust(m, qtable_path, NT_in, T_lo, T_hi, status, &
      ...
      ! PAH population is the graphite fraction xi_gra(a) of HD23 eq. 15 --
      ! there is no PAH scattering cross section, only a PAH absorption one --
      ! so Csca_pah carries xi_gra(a) times the graphite sphere and both
      ! charge states share it.  Leaving it out costs 3.2% of tau_sca at
      ! 0.1 um against the HD23 release and nothing longward of 0.3 um.
   ...
   subroutine build_mrn(m, qtable_path, NT_in, T_lo, T_hi, status, lam_min, &
      ...
      character(len=512) :: q_h5, q_dir, kpath
      ...
      call free_shared_model_arrays()
      ! Drop anything a previous astrodust build left behind rather than leave
      ! a stale array on the wrong grid; the optics below are computed on this
      ! model's own size/wavelength grids.
      if (allocated(gsca_ad))  deallocate(gsca_ad)
      if (allocated(Csca_car)) deallocate(Csca_car, gsca_car)
      ...
      call load_model_extinction_table(m, sed_data_path(KEXT_MRN), &
                                       kext_path, kext_ok, kpath, &
                                       default_h5=sed_data_path(KEXT_H5_MRN))
      if (.not. kext_ok) then
         if (present(status)) then
            status = 3;  return
         else
            write(*,'(a,a)') ' build_mrn: cannot read the extinction table ', trim(kpath)
            stop 1
         end if
      end if
   ...
   ! terms.  Every population of every model here does carry them: in the PAH
   ! populations what scatters is the graphite fraction xi_gra(a) of HD23
   ! eq. 15, since a PAH has an absorption cross section and no scattering one.
   !
