program main_zubko
   ...
   ! silicate with the ZDA size-distribution formula and the DustEM optics and
   ...
   ! is the one shipped model whose optics grid (1.0e-3 um, from the DustEM
   ...
   ! main_dl07.x.
   !
   ! Usage:  ./main_zubko.x [heuristic | draine | qm | equil] [euv]
   !   solver : stochastic solver; default 'heuristic' (the model default).
   !   euv    : replace the Mathis field below the Lyman limit by a hard
   !            power-law component, so that the field really does carry
   !            photons above 13.6 eV. Without it the field stops at the Lyman
   !            limit and the single-photon bound must stay at 13.6 eV no
   !            matter how far the optics grid extends; with it the bound must
   !            follow the field up. Running both is the regression test.
   !
   ! Output: output/zubko_sed_ours[_<solver>][_euv].dat
   ...
   ! Artificial EUV component (only with the 'euv' argument): a diluted 1e5 K
   ...
   real(wp), parameter :: T_EUV = 1.0e5_wp
   real(wp), parameter :: W_EUV = 2.0e-19_wp
   ...
   do iarg = 1, narg
      ...
      case default
         ...
         write(*,'(a)')   ' usage: ./main_zubko.x [heuristic|draine|qm|equil] [euv]'
         stop 1
      end select
   end do
   ...
   write(*,'(a,l1)')   ' EUV component : ', add_euv
   ...
   if (add_euv) then
      do k = 1, m%NLAM
         if (m%lam(k) < LAM_LYMAN) J_lam(k) = W_EUV * bbody(T_EUV, m%lam(k))
      end do
   end if
   ...
   if (add_euv) write(u,'(a,es10.3,a,es10.3)') &
        '# artificial EUV component below the Lyman limit: W*B_lambda(T), T = ', &
        T_EUV, ' K, W = ', W_EUV
