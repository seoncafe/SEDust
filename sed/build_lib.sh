...
#                                  REFUSED with status 11, never answered with
...
# SEDust sources, in dependency order -- the same list as SRC_PIPE in the
# Makefile, plus dust_lib.  `sed_mathlib` is named so upstream to avoid a clash
...
if [ -n "$WITH_TMATRIX" ]; then
   ...
   echo "Built: sed/lib/libsedust.a  (no T-matrix; euv_tmatrix = .true. gives status 11)"
fi
