...
C   verbatim from tmd.lp.f (Mishchenko 2005). Only the main program is
C   modified.
C
C   Edits versus the original main program:
C     - The implicit main is wrapped as SUBROUTINE TMD_ONE with an
C       argument list.
C     - Inline test-case assignments (RAT, NDISTR, AXMAX, NPNAX, B, GAM,
C       NKMAX, EPS, NP, LAM, MRR, MRI, DDELT, NPNA, NDGS) removed.
C       Values come from the argument list. RAT is hard-coded to 1
C       (volume-equivalent sphere); the size-distribution is not used.
C     - File OPEN statements (units 6 'test', 10 'tmatr.write') removed.
C     - The size-distribution loop (DO 600 IAX = 1, NPNAX) and its
C       internal Gaussian quadrature in radius (DO 500 INK = 1, NK,
...
C     INPUT
C       AXI    equivalent-volume-sphere radius [microns]
C       LAM    wavelength [same units as AXI; microns]
C       MRR    Re(refractive index)
C       MRI    Im(refractive index), > 0 for absorption
C       EPS    aspect ratio (b/a > 1 for oblate, < 1 for prolate when
C              NP = -1; cylinder D/L when NP = -2; deformation parameter
C              when NP > 0 Chebyshev order)
C       NP     INTEGER -1 spheroid, -2 finite cylinder,
C              >=0 Chebyshev particle of order NP
C       DDELT  convergence tolerance, recommended 1.D-3
C       NDGS   INTEGER Gauss-quadrature multiplier per NMAX,
C              recommended 2 for compact particles
C     OUTPUT
C       QEXT   extinction efficiency, Cext / (pi * AXI**2)
...
C  Original main program did `DDELT = 0.1D0*DDELT` directly on the input
C  variable. As a wrapper that is now an argument, do this on a local
C  copy DDELT_LOC instead — caller may pass a Fortran PARAMETER (read-only
C  storage), in which case writing to DDELT segfaults with SIGBUS.
      DDELT_LOC = 0.1D0*DDELT

C  Single particle, RAT = 1 (volume-equivalent sphere): A = AXI.
      A    = AXI
      XEV  = 2D0*P*A/LAM
...
C  Q convention: Q = C / (pi * AXI**2). AXI is in microns; CSCA, CEXT
C  carry the LAM**2 factor from COEFF1 above and inherit the units of
...
C  Inner subroutines below copied verbatim from tmd.lp.f.
C***********************************************************************
