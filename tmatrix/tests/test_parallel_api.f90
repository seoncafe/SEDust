program test_parallel_api
   !! Public default-API concurrency test.  Every OpenMP worker owns a
   !! distinct workspace and therefore executes the full-direct backend.
   ...
   if (failures /= 0) error stop 'parallel public API mismatch'
   write(*,'(a,i0,a)') 'PASS: OpenMP public API regression (', nwork, ' independent workspaces)'
